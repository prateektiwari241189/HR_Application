class LinkedinController < ApplicationController
  ...
  def index
    ...
    unless LinkedinOauthSetting.find_by_id(@emp.id).nil?
  ...
  def generate_linkedin_oauth_url
    byebug
    @emp = Employee.find_by(id: params[:format])
    session[:user_id] = Employee.find_by(id: params[:format]).id
    if LinkedinOauthSetting.find_by(id: @emp.id).nil?
      client = LinkedIn::Client.new('iv6uehul4g5m', 'wtMfG2MbFerSULTC', @@config)
